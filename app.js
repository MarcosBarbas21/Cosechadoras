...
const recorrido = () => {
  ...
  do {
    ...
    let lat = Number(newCoordddd.slice(0,newCoordddd.indexOf(',')))
    let lon = Number(newCoordddd.slice(newCoordddd.indexOf(',')+1,newCoordddd.length))
...
const getValidatedCoordinates = (props, coordinates) => {
  ...
  //let validatedCoords = sliceDescription(newData, coordinates);
  let filteredMap = filterMap(newData, lotes);
